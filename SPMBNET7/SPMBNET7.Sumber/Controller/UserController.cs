...
using SPMBNET7.Sumber.ViewModels.Administrations;
...
using static SPMBNET7.Sumber.ViewModels.Administrations.UserClaimsViewModel;

namespace SPMBNET7.Sumber.Controller
{
        ...
        public async Task<IActionResult> Edit(ApplicationUser user, string SelectedJBahagianList, List<string> SelectedRoleList)
        {
            if (SelectedRoleList != null)
            {
                if (ModelState.IsValid)
                {
                    ...
                    int? pekerjaId = _db.applicationUsers.Where(b => b.Id == user.Id).FirstOrDefault()!.SuPekerjaId;
